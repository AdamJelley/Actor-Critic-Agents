...
class Agent():
    def __init__(self, alpha, beta, input_dims, tau, env, gamma=0.99, update_actor_interval=2, warmup=1000,
                    n_actions=2, max_size=int(1e6), layer1_size=400, layer2_size=300, batch_size=100, noise=0.1):
        self.gamma = gamma
        self.tau = tau
    ...
    def learn(self):
        ...
        # This may not work if elements of min and max are not all equal in magnitude
        target_actions = T.clamp(target_actions, self.min_action[0], self.max_action[0])

        new_q1 = self.target_critic_1.forward(new_state, target_actions)
        new_q2 = self.target_critic_2.forward(new_state, target_actions)
        ...
        new_critic_value = T.min(new_q1, new_q2)

        target = reward + self.gamma*new_critic_value
